...
/**
 * Mutates the in-memory runtime default configuration (supports incognito & all devices)
 */
export function updateDefaultPlanConfig(config: Partial<PlanSyncConfig>): void {
  if (config.planImageUrl) DEFAULT_PLAN_CONFIG.planImageUrl = config.planImageUrl;
  if (config.planDriveUrl) DEFAULT_PLAN_CONFIG.planDriveUrl = config.planDriveUrl;
  if (config.googleSheetUrl) DEFAULT_PLAN_CONFIG.googleSheetUrl = config.googleSheetUrl;
  if (config.appsScriptUrl !== undefined) DEFAULT_PLAN_CONFIG.appsScriptUrl = config.appsScriptUrl;
  if (config.opacity !== undefined) DEFAULT_PLAN_CONFIG.opacity = config.opacity;
  if (config.version) DEFAULT_PLAN_CONFIG.version = config.version;
  if (config.lastUpdated) DEFAULT_PLAN_CONFIG.lastUpdated = config.lastUpdated;
}
...
export async function fetchGitHubPlanConfig(): Promise<PlanSyncConfig | null> {
  try {
    ...
    if (data && (data.planImageUrl || data.planDriveUrl || data.googleSheetUrl || data.appsScriptUrl)) {
      const parsedConfig: PlanSyncConfig = {
      ...
      updateDefaultPlanConfig(parsedConfig);
      return parsedConfig;
    }
  } catch (err) {
...
export async function savePlanConfigToServer(
  ...
  // Update runtime in-memory default immediately (works across tabs and incognito)
  updateDefaultPlanConfig(incomingConfig);
  ...
  if (incomingConfig.appsScriptUrl !== undefined) {
    try {
      if (incomingConfig.appsScriptUrl) {
        localStorage.setItem('silpa_bhirasri_apps_script_url', incomingConfig.appsScriptUrl);
      } else {
        localStorage.removeItem('silpa_bhirasri_apps_script_url');
      }
    } catch {}
  }
  ...
  try {
    ...
    if (res.ok) {
      ...
      if (data && data.success && data.config) {
        updateDefaultPlanConfig(data.config);
