  ...
  try {
    ...
    if (data) {
      ...
      if (parsed && parsed.seats && parsed.metadata) {
        ...
        // Clean up any legacy "ที่นั่งสำรอง" or "สำรอง" placeholder strings for empty seats
        Object.keys(parsed.seats).forEach(seatId => {
          const s = parsed.seats[seatId];
          if (!s.guestName && s.position && (s.position.includes('ที่นั่งสำรอง') || s.position.includes('สำรอง') || s.position === 'ว่าง')) {
            s.position = '';
            if (s.organization && (s.organization.includes('ที่นั่งสำรอง') || s.organization.includes('สำรอง') || s.organization === 'ว่าง')) {
              s.organization = '';
            }
          }
        });
