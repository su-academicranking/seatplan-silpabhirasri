...
export const SvgSeatCard: React.FC<SvgSeatCardProps> = ({
  ...
  // Check if position text is just a reserve/empty placeholder
  const isReservePlaceholder = (text?: string): boolean => {
    if (!text) return true;
    const t = text.trim().toLowerCase();
    return t === '' || t.includes('ที่นั่งสำรอง') || t.includes('สำรอง') || t === 'ว่าง';
  };

  const hasName = Boolean(seat.guestName && seat.guestName.trim());
  // Single unified Pos/Org text matching Google Sheet
  const rawPosOrg = !isReservePlaceholder(seat.position) ? (seat.position || seat.organization || '') : (seat.organization || '');
  const cleanPosOrg = !isReservePlaceholder(rawPosOrg) ? rawPosOrg.trim() : '';
  const displayPosOrg = cleanPosOrg !== (seat.guestName || '').trim() ? cleanPosOrg : '';
  const hasSubContent = Boolean(displayPosOrg);

  const isEmpty = (!hasName && !hasSubContent) || seat.status === 'empty';
  ...
  // Subtitle matching Google Sheet "ตำแหน่ง/สังกัด"
  const subtitleText = displayPosOrg;

  // Wrap text lines safely (allow up to 4 lines so full names from Google Sheet are displayed)
  const nameLines = hasName ? wrapSvgText(seat.guestName!, maxVisualChars, 4) : [];
  const posLines = hasName && subtitleText
    ? wrapSvgText(subtitleText, maxVisualChars + 1, 3)
    : [];
  const soloPosLines = !hasName && displayPosOrg
    ? wrapSvgText(displayPosOrg, maxVisualChars + 1, 3)
    : [];
  ...
  const bodyLinesCount = isEmpty
    ? 1
    : isAwardeeNumber
      ? 1 + nameLines.length + posLines.length + (!hasName && !hasSubContent ? 1 : 0)
