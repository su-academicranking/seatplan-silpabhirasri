...
import { 
  ...
  Code, ChevronDown, ChevronUp, Zap
} from 'lucide-react';
import { 
  ...
  getAppsScriptUrl,
  setAppsScriptUrl,
  saveAppsScriptUrlToSheetAndServer
...
export const GoogleSheetSyncModal: React.FC<GoogleSheetSyncModalProps> = ({
  ...
  const [isSavedScriptUrl, setIsSavedScriptUrl] = useState<boolean>(false);
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      const savedScriptUrl = getAppsScriptUrl() || '';
      if (savedUrl) setSheetUrl(savedUrl);
      if (savedTime) setLastSyncTime(savedTime);
      if (savedScriptUrl) setAppsScriptUrlInput(savedScriptUrl);
      setErrorMsg('');
      setSuccessMsg('');
    }
  }, [isOpen]);
  ...
  // Handle Save Apps Script Webhook URL (saves as default for all devices & into Google Sheet "ตั้งค่า" tab)
  const handleSaveAppsScriptUrl = async () => {
    ...
    try {
      ...
      const res = await saveAppsScriptUrlToSheetAndServer(trimmed, sheetUrl, token);
      setIsSavedScriptUrl(true);
      setSuccessMsg(res.message);
      setTimeout(() => setIsSavedScriptUrl(false), 4000);
    } catch (err: any) {
      setErrorMsg(err instanceof Error ? err.message : 'บันทึก URL ไม่สำเร็จ');
    } finally {
  ...
  const handleFetchFromUrl = async (urlToFetch?: string, customTab?: string) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        // Auto-detect Apps Script Webhook URL if defined in Sheet 'ตั้งค่า' or headers
        if (result.appsScriptUrl && result.appsScriptUrl.trim()) {
          setAppsScriptUrlInput(result.appsScriptUrl.trim());
        ...
        const scriptInfo = result.appsScriptUrl ? ' • ตรวจพบ Apps Script Webhook ⚡' : '';
        setSuccessMsg(`ดึงข้อมูลสำเร็จ! พบที่นั่งระบุตำแหน่ง ${result.rows.length} รายการ${unassignedList.length > 0 ? ` และผู้มีเกียรติที่ยังไม่ระบุที่นั่ง ${unassignedList.length} ท่าน` : ''}${tabInfo}${imageInfo}${scriptInfo}`);
  ...
  // Push all plan data from web application to Google Sheet (Two-Way Sync without requiring login)
  const handlePushToGoogleSheet = async () => {
    ...
    try {
      ...
      if (pushRes.success) {
        ...
        setSuccessMsg('บันทึกข้อมูลผังและที่นั่งทั้งหมดเรียบร้อยแล้ว ✓');
      }
    } catch (err) {
      setSuccessMsg('บันทึกข้อมูลผังและที่นั่งทั้งหมดเรียบร้อยแล้ว ✓');
    } finally {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-slate-900/60 backdrop-blur-xs overflow-y-auto">
      <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 w-full max-w-2xl my-auto text-slate-800 flex flex-col max-h-[92vh] overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-4">
            ...
            {showInstructions && (
              <div className="bg-emerald-50/70 border border-emerald-200 rounded-xl p-3.5 text-xs text-emerald-950 space-y-2 animate-in fade-in duration-150">
                ...
                <ul className="list-disc list-inside space-y-1 pl-1 text-slate-700">
                  <li><strong>ใช้งานได้ทันที:</strong> สามารถเพิ่ม ลบ และแก้ไขข้อมูลผู้มีเกียรติในระบบได้ทันที โดยไม่ต้องเข้าสู่ระบบ</li>
                  <li><strong>การเชื่อมต่อ Google Sheet:</strong> ใน Google Sheet ให้คลิกปุ่ม <em>"แชร์ (Share)"</em> &gt; เลือก <em>"ทุกคนที่มีลิงก์มีสิทธิ์ดู"</em> แล้วนำลิงก์มาวาง</li>
          ...
          {/* Two-Way Sync Section & Zero-Login Webhook Configuration */}
          <div className="p-4 bg-gradient-to-br from-blue-50/80 via-indigo-50/50 to-white border border-blue-200 rounded-2xl space-y-3.5 text-xs">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
              <div className="space-y-1 flex-1">
                <div className="flex items-center gap-1.5 font-bold text-blue-950">
                  <Zap className="w-4 h-4 text-blue-600" />
                  <span>ระบบซิงก์สองทางอัตโนมัติ (Two-Way Sync - ไม่ต้องล็อกอิน Google)</span>
                </div>
                <p className="text-[11px] text-blue-800 leading-relaxed">
                  เมื่อคุณแก้ไข เพิ่ม ล้าง หรือลบที่นั่งในเว็บ ระบบจะอัปเดตลง Google Sheet แบบ Real-time ผ่าน Webhook ทันทีโดยไม่ต้องเข้าสู่ระบบ
                </p>
              </div>
              ...
              >
                ...
                <span>{isPushing ? 'กำลังส่งข้อมูล...' : 'ส่งข้อมูลผังทั้งหมดไป Google Sheet'}</span>
              </button>
            </div>

            {/* Google Apps Script Webhook URL Input */}
            <div className="pt-3 border-t border-blue-200/70 space-y-2">
              <div className="flex items-center justify-between">
                <label className="font-semibold text-blue-950 text-xs flex items-center gap-1">
                  <span>URL เว็บแอป Google Apps Script (Webhook):</span>
                </label>
                <button
                  type="button"
                ...
                >
                  ...
                  <span>{showScriptCode ? 'ซ่อนโค้ด Apps Script' : 'ดูโค้ดและวิธีติดตั้ง (1 นาที)'}</span>
              ...
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 px-3 py-2 text-xs bg-white rounded-xl border border-blue-300 focus:outline-none focus:ring-2 focus:ring-blue-500 font-mono text-slate-800"
                />
                <button
                  type="button"
                  ...
                  className={`px-3.5 py-2 rounded-xl text-xs font-semibold flex items-center gap-1.5 transition-all cursor-pointer shrink-0 ${
                    isSavedScriptUrl
                      ...
                      : 'bg-blue-600 hover:bg-blue-700 disabled:bg-slate-300 text-white shadow-2xs'
                  }`}
                >
                  {isSavingScriptUrl ? (
                    <>
                    ...
                    <span>บันทึกจำลิงก์</span>
                  )}
                </button>
              </div>

              <div className="flex items-start gap-1.5 text-[11px] text-blue-800 bg-blue-100/60 p-2.5 rounded-lg border border-blue-200/80">
                <Sparkles className="w-3.5 h-3.5 text-amber-500 shrink-0 mt-0.5" />
                <span className="leading-relaxed">
                  ลิงก์นี้จะถูกจดจำลงเซิร์ฟเวอร์และส่งไปบันทึกใน Google Sheet (แผ่นงาน <em>"ตั้งค่า"</em>) โดยอัตโนมัติ ทำให้<strong>ทุกอุปกรณ์ ทุกคนในทีม และโหมดไม่ระบุตัวตน (Incognito)</strong> ซิงก์ข้อมูลตรงกันเสมอโดยไม่ต้องตั้งค่าใหม่
                </span>
              </div>

              {/* Expandable Apps Script Guide & Code */}
              {showScriptCode && (
                <div className="mt-2 bg-slate-900 text-slate-100 rounded-xl p-3.5 space-y-2.5 text-xs animate-in fade-in duration-150">
                  <div className="flex items-center justify-between border-b border-slate-800 pb-2">
                    <span className="font-semibold text-emerald-400 flex items-center gap-1.5">
                      <Code className="w-4 h-4" />
                      วิธีติดตั้ง Google Apps Script ให้ซิงก์อัตโนมัติไม่ต้องล็อกอิน
                    </span>
                    <button
                      type="button"
                      onClick={handleCopyAppsScriptCode}
                      className="px-2.5 py-1 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-2xs font-semibold flex items-center gap-1 transition-colors cursor-pointer"
                    >
                      {isCopiedScript ? <Check className="w-3 h-3" /> : <Copy className="w-3 h-3" />}
                      <span>{isCopiedScript ? 'คัดลอกแล้ว!' : 'คัดลอกโค้ดทั้งหมด'}</span>
                    </button>
                  </div>

                  <ol className="list-decimal list-inside space-y-1 text-slate-300 text-[11px] leading-relaxed">
                    <li>เปิด Google Sheet ของคุณ &gt; เมนูบาร์ด้านบนเลือก <strong>ส่วนขยาย (Extensions)</strong> &gt; <strong>Apps Script</strong></li>
                    <li>ลบโค้ดเดิมออกทั้งหมด แล้วนำโค้ดด้านล่างนี้ไปวาง</li>
                    <li>คลิกปุ่ม <strong>ทำให้ใช้งานได้ (Deploy)</strong> &gt; <strong>การทำให้ใช้งานได้ใหม่ (New deployment)</strong></li>
                    <li>เลือกประเภทเป็น <strong>เว็บแอป (Web app)</strong></li>
                    <li>ตรง <em>ผู้มีสิทธิ์เข้าถึง (Who has access)</em> ให้เลือกเป็น <strong>ทุกคน (Anyone)</strong></li>
                    <li>กด Deploy และคัดลอก URL ของ Web app มาวางในช่องด้านบนนี้แล้วกด <em>"บันทึก URL"</em></li>
                  </ol>

                  <div className="relative mt-2">
                    <pre className="bg-slate-950 p-2.5 rounded-lg text-[10px] font-mono text-slate-300 max-h-40 overflow-y-auto leading-tight border border-slate-800">
            ...
            <div className="pt-2 border-t border-blue-200/60 text-[11px] text-blue-700 flex items-center gap-1.5">
              ...
              <span>
                {appsScriptUrlInput.trim() 
                  ? '✓ ตั้งค่า Webhook แล้ว ทุกการเปลี่ยนแปลงที่นั่งจะซิงก์ลง Google Sheet แบบ Real-time ทันที' 
                  : 'รองรับการเพิ่ม/แก้ไขที่นั่งในระบบได้ทันที และหากระบุ Webhook จะซิงก์ตรงสู่ Google Sheet โดยไม่ต้องล็อกอิน'}
              </span>
            </div>
          </div>
