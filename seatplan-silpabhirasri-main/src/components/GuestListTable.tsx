...
interface GuestListTableProps {
  ...
  onPushAllToGoogleSheet?: () => Promise<void>;
  onDirectGoogleSignIn?: () => Promise<void>;
}
...
export const GuestListTable: React.FC<GuestListTableProps> = ({
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200 shadow-xs overflow-hidden flex flex-col">
      ...
      {/* Two-Way Google Sheet Sync Status Banner */}
      <div className="px-4 py-2 border-b text-xs flex flex-wrap items-center justify-between gap-2 transition-colors bg-emerald-50/90 border-emerald-200 text-emerald-900">
        <div className="flex items-center gap-2">
          <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse shrink-0" />
          <span className="font-semibold text-emerald-800">
            ระบบเชื่อมกับ Google Sheet อัตโนมัติ (การแก้ไขในระบบจะบันทึกตรงไปยัง Google Sheet โดยอัตโนมัติ)
          </span>
          {lastSyncedTime && (
        ...
        <div className="flex items-center gap-1.5 ml-auto">
          ...
          <div className="flex items-center gap-1.5 px-2 py-1 bg-emerald-50 text-emerald-800 border border-emerald-200/80 rounded-lg text-xs font-medium">
            <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />
            <span className="hidden sm:inline">เชื่อมต่อ Google Sheet อัตโนมัติ</span>
            <span className="sm:hidden">ซิงก์ชีตอัตโนมัติ</span>
          </div>

          {onPushAllToGoogleSheet && (
            <button
              type="button"
              onClick={onPushAllToGoogleSheet}
              disabled={isSyncingToSheet}
              className="px-2.5 py-1 bg-emerald-600 hover:bg-emerald-700 active:scale-95 text-white rounded-lg text-xs font-semibold shadow-2xs transition-all flex items-center gap-1 cursor-pointer disabled:opacity-50"
              title="ส่งข้อมูลที่นั่งและผู้มีเกียรติทั้งหมดจากระบบไปยัง Google Sheet"
            >
              <FileSpreadsheet className="w-3.5 h-3.5" />
              <span>{isSyncingToSheet ? 'กำลังส่งข้อมูล...' : 'ส่งข้อมูลทั้งหมดลงชีต'}</span>
            </button>
          )}
          ...
          {onOpenGoogleSheets && (
            <button
              ...
              className="px-2 py-1 text-slate-600 hover:text-slate-900 hover:bg-white/70 rounded-lg text-xs font-medium transition-colors cursor-pointer border border-transparent hover:border-slate-200"
            >
              ตั้งค่าชีต
            </button>
          )}
                  ...
                  {filteredSeats.length === 0 ? (
                    ...
                    filteredSeats.map((seat) => (
                      ...
                      >
                        ...
                        {/* Position / Organization (Inline edit, Left-aligned, full width & text) */}
                        <td className="py-2.5 px-3.5 text-left text-slate-700">
                          <input
                            ...
                            title={seat.organization || 'ยังไม่ระบุตำแหน่ง/สังกัด'}
                          />
                        </td>
