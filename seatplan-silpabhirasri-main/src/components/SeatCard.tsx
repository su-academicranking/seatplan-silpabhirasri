...
export const SeatCard: React.FC<SeatCardProps> = ({
  ...
  return (
    <div
      ...
      `}
      title={isEmpty ? `${seat.id} : (ว่าง)` : `${seat.id} : ${seat.guestName ? `${seat.guestName} ` : ''}${displayPosOrg ? `(${displayPosOrg}) ` : ''}(${seat.setGroup || 'ไม่มี Set'}${seat.hasArtSet ? ' | มี Art Set' : ''}${seat.hasFlowerBasket ? ' | วางกระเช้า' : ''})`}
    >
      ...
      {/* Main Content: Full Position, Full Guest Name & Organization */}
          ...
          <div className="w-full flex flex-col items-center justify-center text-center">
            {hasName ? (
              <div className="flex flex-col items-center justify-center w-full">
                ...
                {/* Subtitle: Unified Position / Organization matching Google Sheet */}
                {displayPosOrg && (
